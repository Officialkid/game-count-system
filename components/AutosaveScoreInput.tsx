...
export function AutosaveScoreInput({
  ...
  const getInputClassName = (hasValue: boolean, hasError: boolean) => {
    if (hasError) {
      return 'w-full px-3 py-2 border-2 border-red-500 dark:border-red-600 rounded-md focus:ring-2 focus:ring-red-500 dark:focus:ring-red-600 focus:border-red-500 dark:focus:border-red-600 dark:bg-gray-700 dark:text-gray-100';
    }
    if (hasValue) {
      return 'w-full px-3 py-2 border-2 border-green-500 dark:border-green-600 rounded-md focus:ring-2 focus:ring-green-500 dark:focus:ring-green-600 focus:border-green-500 dark:focus:border-green-600 dark:bg-gray-700 dark:text-gray-100';
    }
    return 'w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:text-gray-100';
  };

  return (
    <Card className="dark:bg-gray-800 dark:border-gray-700">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Add Score</h3>
        {saving && (
          <div className="flex items-center gap-2 text-sm text-primary-600 dark:text-primary-400" role="status" aria-live="polite">
        ...
        {saved && !saving && (
          <div className="flex items-center gap-2 text-sm text-green-600 dark:text-green-400" role="status" aria-live="polite">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div>
          <label htmlFor="game-number" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            Game # <span className="text-red-500 dark:text-red-400">*</span>
          </label>
          <Input
            id="game-number"
            type="number"
        ...
        <div>
          <label htmlFor="points" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            Points <span className="text-red-500 dark:text-red-400">*</span>
          </label>
          <Input
            id="points"
            type="number"
      ...
      {error && (
        <div 
          id="score-error"
          className="mt-4 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md text-red-700 dark:text-red-400 text-sm" 
          role="alert"
          aria-live="assertive"
        >
          {error}
